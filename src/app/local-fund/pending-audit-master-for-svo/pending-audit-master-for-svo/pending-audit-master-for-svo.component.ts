...
@Component({
  selector: 'app-pending-audit-master-for-svo',
  templateUrl: './pending-audit-master-for-svo.component.html',
  styleUrls: ['./pending-audit-master-for-svo.component.css']
})
export class PendingAuditMasterForSvoComponent implements OnInit {
  // variables
  ...
  todayDate = Date.now();
  // form group
  pendingAuditMasterForSvo: FormGroup;
  ...
  ngOnInit() {
    ...
    this.pendingAuditMasterForSvo = this.fb.group({
      financialYear: [''],
      districtName: [''],
      talukaName: [''],
      instituteName: [''],
  ...
  selectDistrict() {
    const district = this.pendingAuditMasterForSvo.value.districtName;
  ...
  // add table entry
  add() {
    if (this.pendingAuditMasterForSvo.valid) {
      this.isSubmitted = false;

      this.elementData.push({
        financialYear: this.financialYearList[this.pendingAuditMasterForSvo.value.financialYear].viewValue,
        district: this.districtNameList[this.pendingAuditMasterForSvo.value.districtName].viewValue,
        taluka: this.talukaNameList[this.pendingAuditMasterForSvo.value.talukaName].viewValue,
        instituteName: this.instituteNameList[this.pendingAuditMasterForSvo.value.instituteName].viewValue,
        auditType: this.auditTypeList[this.pendingAuditMasterForSvo.value.auditType].viewValue,
        sanctionedMandays: '25'
      });
  ...
  // reset form
  resetForm() {
    this.pendingAuditMasterForSvo.reset();

    // patches defalut values
    this.pendingAuditMasterForSvo.patchValue({
      sanctionedMandays: '25'
    });
  }
