import { FormGroup, FormBuilder, FormControl } from '@angular/forms';
import { Component, OnInit } from '@angular/core';
import { Router } from '@angular/router';
import { MatDialog, MatTableDataSource } from '@angular/material';
import { InwardDetailsListing } from 'src/app/model/dppf-hba';
import { dppfHbaMessage } from 'src/app/common/error-message/common-message.constants';
import { CommonListing } from 'src/app/model/common-listing';

@Component({
  selector: 'app-inward-details-listing',
  templateUrl: './inward-details-listing.component.html',
  styleUrls: ['./inward-details-listing.component.css']
})
export class InwardDetailsListingComponent implements OnInit {
  // Date
  todayDate = Date.now();
  maxDate = new Date();
  // Form Group
  inwardDetailsForm: FormGroup;
  // Form Control
  hbaMcaCtrl: FormControl = new FormControl();
  districtCtrl: FormControl = new FormControl();
  mpnthCtrl: FormControl = new FormControl();
  yearCtrl: FormControl = new FormControl();
  credCtrl: FormControl = new FormControl();
  inwardTypeCtrl: FormControl = new FormControl();
  creditCtrl: FormControl = new FormControl();
  public errorMessages;
  // List
  inwarde_list: CommonListing[] = [
    { value: '1', viewValue: 'Top Schedule' },
    { value: '3', viewValue: 'Others' },
    { value: '4', viewValue: 'Cheque / Demand Draft' },
    { value: '5', viewValue: 'Request for Account Generation' },
    { value: '6', viewValue: 'Demand for Interest Conformation Letter' },
    { value: '7', viewValue: 'Demand for NDC' },
    { value: '8', viewValue: 'Demand for refund order' },
    { value: '9', viewValue: 'AGTE Misclassified Entry' },
    { value: '10', viewValue: 'AGTE Clearance Entry(Sent by DPPF)' },
    { value: '11', viewValue: 'Debit/Credit Correction Entry' },
    { value: '12', viewValue: 'AG CA Figure' }
  ];
  credit_list: CommonListing[] = [
    { value: '1', viewValue: 'Credit' },
    { value: '2', viewValue: 'Debit' }
  ];

  year_list: CommonListing[] = [
    { value: '1', viewValue: '2009' },
    { value: '2', viewValue: '2010' },
    { value: '3', viewValue: '2011' },
    { value: '4', viewValue: '2012' },
    { value: '5', viewValue: '2013' },
    { value: '6', viewValue: '2014' },
    { value: '7', viewValue: '2015' },
    { value: '8', viewValue: '2016' },
    { value: '9', viewValue: '2017' },
    { value: '10', viewValue: '2018' },
    { value: '11', viewValue: '2019' },
    { value: '12', viewValue: '2020' },
  ];
  month_list: CommonListing[] = [
    { value: '1', viewValue: 'Jan' },
    { value: '2', viewValue: 'Feb' },
    { value: '3', viewValue: 'Mar' },
    { value: '4', viewValue: 'Apr' },
    { value: '5', viewValue: 'May' },
    { value: '6', viewValue: 'Jun' },
    { value: '7', viewValue: 'Jul' },
    { value: '8', viewValue: 'Aug' },
    { value: '9', viewValue: 'Sep' },
    { value: '10', viewValue: 'Oct' },
    { value: '11', viewValue: 'Nov' },
    { value: '12', viewValue: 'Dec' },

  ];
  load_list: CommonListing[] = [
    { value: '1', viewValue: 'HBA' },
    { value: '2', viewValue: 'MCA' },
  ];
  districtList: CommonListing[] = [
    { value: '1', viewValue: 'District Treasury Office,Gandhinagar' },
    { value: '2', viewValue: 'District Treasury Office,Ahmedabad' },
    { value: '3', viewValue: 'District Treasury Office,Rajkot' },
  ];
  // Table Source

  Element_Data: InwardDetailsListing[] = [
    {
      srno: '1',
      inwardNo: 'TSC/8/2020/1201',
      dateInward: '22-Feb-2016',
      treasuryPao: 'Treasury',
      empNo: '1000000001',
      empName: 'Mr. Narandrabhai Patel',
      year: '2020',
      month: 'Aug',
      paymentType: 'Credit',
      inwardType: 'AG CA Figure',
      district: 'Ahmedabad',
    },

    {
      srno: '2',
      inwardNo: 'TSC/8/2020/1201',
      dateInward: '12-Feb-2016',
      treasuryPao: 'PAO',
      empNo: '1000000002',
      empName: 'Mr. Narandrabhai Patel',
      year: '2020',
      month: 'Aug',
      paymentType: 'Debit',
      inwardType: 'Request for Account Generation',
      district: 'Gandhinagar',
    },

    {
      srno: '3',
      inwardNo: 'TSC/8/2020/1201',
      dateInward: '02-Feb-2016',
      treasuryPao: 'Treasury',
      empNo: '1000000003',
      empName: 'Mr. Narandrabhai Patel',
      year: '2020',
      month: 'Aug',
      paymentType: 'Credit',
      inwardType: 'AGTE Misclassified Entry',
      district: 'Ahmedabad',
    },

    {
      srno: '4',
      inwardNo: 'TSC/8/2020/1201',
      dateInward: '22-Feb-2016',
      treasuryPao: 'PAO',
      empNo: '1000000004',
      empName: 'Mr. Narandrabhai Patel',
      year: '2020',
      month: 'Aug',
      paymentType: 'Debit',
      inwardType: 'Top Schedule',
      district: 'Gandhinagar',
    },


  ];

  displayedColumns: string[] = [
    'srno',
    'inwardNo',
    'inwardType',
    'dateInward',
    'district',
    'treasuryPao',
    'year',
    'month',
    'action'

  ];

  dataSource = new MatTableDataSource<InwardDetailsListing>(this.Element_Data);

  constructor(private router: Router, public dialog: MatDialog, private fb: FormBuilder) { }

  ngOnInit() {

    this.inwardDetailsData();
  }
  inwardDetailsData() {
    this.errorMessages = dppfHbaMessage;
    this.inwardDetailsForm = this.fb.group({
      inwardType: [''],
      empNo: [''],
      empName: [''],
      month: [''],
      year: [''],
      credir: [''],
      district: [''],
      hbaMczNo: [''],
      inwardNo: [''],
      inwardFromDate: [''],
      inwardToDate: [''],
      mcano: ['']
    });
  }
  navigate() {
    this.router.navigate(['./dppf-hba/inward-details']);
  }
  onInwardType(value) {
    if (value === '5' || value === '8') {
      this.displayedColumns = [
        'srno',
        'inwardNo',
        'inwardType',
        'empNo',
        'empName',
        'dateInward',
        'district',
        'treasuryPao',
        'year',
        'month',
        'action'
      ];
    } else if (value === '6' || value === '7') {
      this.displayedColumns = [
        'srno',
        'inwardNo',
        'inwardType',
        'empNo',
        'empName',
        'dateInward',
        'district',
        'treasuryPao',
        'action'
      ];
    } else if (value === '2' || value === '3' || value === '4' || value === '11') {
      this.displayedColumns = [
        'srno',
        'inwardNo',
        'inwardType',
        'dateInward',
        'district',
        'treasuryPao',
        'action'
      ];
    } else {
      this.displayedColumns = [
        'srno',
        'inwardNo',
        'inwardType',
        'dateInward',
        'district',
        'treasuryPao',
        'year',
        'month',
        'action'
      ];
    }
  }


}
