...
import { ListValue, AdviceAuditor, EpayTable, DetailPostingDate } from 'src/app/model/pdpla';

@Component({
  selector: 'app-advice-auditor',
  templateUrl: './advice-auditor.component.html',
  styleUrls: ['./advice-auditor.component.css']
})
export class AdviceAuditorComponent implements OnInit {
  // Date
  todayDate = Date.now();
  today = new Date();
  ...
  // sets value on change of payment type
  payment(data) {
    if (data.value === '1') {
      this.epay = false;
      this.cheque = true;
