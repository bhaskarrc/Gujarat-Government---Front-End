...
import { AchieveAgainstTarget } from 'src/app/model/budget';
...
@Component({
  selector: 'app-achievement-against-targets',
  templateUrl: './achievement-against-targets.component.html',
  styleUrls: ['./achievement-against-targets.component.css']
})
export class AchievementAgainstTargetsComponent implements OnInit {
...
// Listing table
AchievementAgainstDataColumn: string[] = [
    'srno', 'financialYear', 'department', 'bpnCode', 'refNum', 'refDate',
    'recFromOn', 'sentToON', 'lyingWith', 'wStatus', 'status', 'action'
  ];

  AchievementAgainstDataSource = new MatTableDataSource(this.AchievementAgainstData);
...
// On search click
  ...
  goToScreen() {
    this.router.navigate(['/budget/achievement-against-targets-details']);
  }
