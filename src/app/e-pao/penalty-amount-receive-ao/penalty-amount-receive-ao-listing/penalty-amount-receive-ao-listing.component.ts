...
@Component({
  selector: 'app-penalty-amount-receive-ao-listing',
  templateUrl: './penalty-amount-receive-ao-listing.component.html',
  styleUrls: ['./penalty-amount-receive-ao-listing.component.css']
})
export class PenaltyAmountReceiveAoListingComponent implements OnInit {
  // Date
  todayDate = Date.now();
  ...
  // Form Group
  penaltyAmountAoListForm: FormGroup;
  ...
  ngOnInit() {
    ...
    this.penaltyAmountAoListForm = this.fb.group({
      fdate: [''],
      tdate: [''],
      bankType: [''],
      bank: [''],
  ...
  onSave() {
    if (this.penaltyAmountAoListForm.valid) {
  ...
  clearForm() {
    this.penaltyAmountAoListForm.reset();
  }
