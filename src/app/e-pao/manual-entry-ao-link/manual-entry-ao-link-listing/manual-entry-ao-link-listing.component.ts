...
@Component({
  selector: 'app-manual-entry-ao-link-listing',
  templateUrl: './manual-entry-ao-link-listing.component.html',
  styleUrls: ['./manual-entry-ao-link-listing.component.css']
})
export class ManualEntryAoLinkListingComponent implements OnInit {
  todayDate = Date.now();
  ...
  entryMasterAoLinkListForm: FormGroup;
  ...
  ngOnInit() {
    ...
    this.entryMasterAoLinkListForm = this.fb.group({
      refNo: [''],
      eDate: [''],
      debitHead: [''],
      cinNo: [''],
  ...
  clearForm() {
    this.entryMasterAoLinkListForm.reset();
  }

  onSave() {
    if (this.entryMasterAoLinkListForm.valid) {
