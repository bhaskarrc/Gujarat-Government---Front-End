...
@Component({
  selector: 'app-generate-mode-ao',
  templateUrl: './generate-mode-ao.component.html',
  styleUrls: ['./generate-mode-ao.component.css']
})
export class GenerateModeAoComponent implements OnInit {
  ...
  navigate() {
    this.router.navigate(['./e-pao/generate-moe-ao-listing']);
  }
  ...
  openView() {
    const dialogRef = this.dialog.open(GSTDialogAoComponent, {
      width: '1200px',
    });
...
export class GSTDialogAoComponent {
  constructor(
    private router: Router,
    public dialogRef: MatDialogRef<GSTDialogAoComponent>
  ) { }
  newdataSource = new MatTableDataSource<any>(ELEMENT_DATA);
  newdisplayedColumns: string[] = ['srNo', 'gstIN', 'partyName', 'cin', 'paymentDate', 'gstAmount',
