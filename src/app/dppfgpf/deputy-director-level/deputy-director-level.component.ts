...
@Component({
  selector: 'app-deputy-director-level',
  templateUrl: './deputy-director-level.component.html',
  styleUrls: ['./deputy-director-level.component.css']
})
export class DeputyDirectorLevelComponent implements OnInit {
  // VAriable
  errorMessage;
  // Date
  todayDate = Date.now();
  maxDate = new Date();
  // Form Group
  public deputyDirectorLevelForm: FormGroup;
  // Form COntrol
  ...
  ngOnInit() {
    ...
    this.deputyDirectorLevelForm = this.fb.group({
  ...
  // reset form
  resetForm() {
    this.deputyDirectorLevelForm.reset();
  }

  // Open DDLAccountDetailsDialog
  openInwardDialog(event?) {
    // tslint:disable-next-line: no-use-before-declare
    const dialogRef = this.dialog.open(DDLAccountDetailsDialog, {
      width: '1200px',
      height: 'auto',
    }
    );
    dialogRef.afterClosed().subscribe(result => {
      this.deputyDirectorLevelForm.patchValue({
      ...
      this.deputyDirectorLevelForm.get('dateInward').patchValue(this.formatDate(new Date()));
...
export class DDLAccountDetailsDialog implements OnInit {
  ...
  constructor(
    ...
    public dialogRef: MatDialogRef<DDLAccountDetailsDialog>,
