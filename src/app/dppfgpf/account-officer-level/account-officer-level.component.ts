...
@Component({
  selector: 'app-account-officer-level',
  templateUrl: './account-officer-level.component.html',
  styleUrls: ['./account-officer-level.component.css']
})
export class AccountOfficerLevelComponent implements OnInit {
  // VAriable
  errorMessage;
  todayDate = Date.now();
  maxDate = new Date();
  // Form Group
  public accountOfficerLevelForm: FormGroup;
  // Form COntrol
  ...
  ngOnInit() {
    ...
    this.accountOfficerLevelForm = this.fb.group({
  ...
  // reset form
  resetForm() {
    this.accountOfficerLevelForm.reset();
  }

  // Open AOLAccountDetailsDialog
  openInwardDialog(event?) {
    // tslint:disable-next-line: no-use-before-declare
    const dialogRef = this.dialog.open(AOLAccountDetailsDialog, {
      width: '1200px',
      height: 'auto',
    }
    );
    dialogRef.afterClosed().subscribe(result => {
      this.accountOfficerLevelForm.patchValue({
      ...
      this.accountOfficerLevelForm.get('dateInward').patchValue(new Date());
...
export class AOLAccountDetailsDialog implements OnInit {
  ...
  constructor(
    ...
    public dialogRef: MatDialogRef<AOLAccountDetailsDialog>,
