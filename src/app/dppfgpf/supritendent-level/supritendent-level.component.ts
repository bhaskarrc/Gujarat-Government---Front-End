...
@Component({
  selector: 'app-supritendent-level',
  templateUrl: './supritendent-level.component.html',
  styleUrls: ['./supritendent-level.component.css']
})
export class SupritendentLevelComponent implements OnInit {
  // Variable
  errorMessage;
  // DAte
  todayDate = Date.now();
  maxDate = new Date();
  // Form Group
  public supritendentLevelForm: FormGroup;
  // Form COntrol
  ...
  ngOnInit() {
    ...
    this.supritendentLevelForm = this.fb.group({
  ...
  // reset form
  resetForm() {
    this.supritendentLevelForm.reset();
  }

  // Open SLAccountDetailsDialog
  openInwardDialog(event?) {
    // tslint:disable-next-line: no-use-before-declare
    const dialogRef = this.dialog.open(SLAccountDetailsDialog, {
      width: '1200px',
      height: 'auto',
    ...
    dialogRef.afterClosed().subscribe(result => {
      this.supritendentLevelForm.patchValue({
      ...
      this.supritendentLevelForm.get('dateInward').patchValue(this.formatDate(new Date()));
...
export class SLAccountDetailsDialog implements OnInit {
  ...
  constructor(
    ...
    public dialogRef: MatDialogRef<SLAccountDetailsDialog>,
