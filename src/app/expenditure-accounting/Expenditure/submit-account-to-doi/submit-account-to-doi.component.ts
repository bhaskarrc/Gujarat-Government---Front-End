...
@Component({
  selector: 'app-submit-account-to-doi',
  templateUrl: './submit-account-to-doi.component.html',
  styleUrls: ['./submit-account-to-doi.component.css']
})
export class SubmitAccountToDoiComponent implements OnInit {
  directiveObject = new EaDirectives(this.dialog);
  initiatiomdate = new Date((new Date()));
  errorMessages = eaMessage;
  isSubmitted = false;
  // FormGroup
  submitAccountDoiForm: FormGroup;
  ...
  ngOnInit() {
    ...
    this.submitAccountDoiForm = this.fb.group({
      // FormGroup Fields
      year: '',
      month: '',
  ...
  onSave() {
    if (this.submitAccountDoiForm.valid) {
